function MS_SMM_BMS_fams(BMS, mask, extent)
% _
...
%  Last edit: 22/04/2020, 19:25 (V1.4/V20)
...
for i = 1:M
    ...
    fam_names{i} = alpha_img(1:strfind(alpha_img,'_family_')-1);
end;
clear alpha_img
...
%-------------------------------------------------------------------------%
cd(SMM_dir);
H.fname   = strcat('MS_SMM_fam.nii');       % selected model
...
for i = 1:sel_mods                          % selected models
    H.fname   = strcat('MS_SMM_map','_pos_',MF_int2str0(i,ceil(log10(M+1))),'_fam_',MF_int2str0(mod_list(i,1),ceil(log10(M+1))),'_',fam_names{mod_list(i,1)},'.nii');
    H.descrip = sprintf('MS_SMM_BMS: LF in voxels where %s has been selected', fam_names{i});
